...
namespace Print4e.Common
{
	...
	public sealed class RichTextColumns : Panel
	{
		...
		/// </summary>
		public static readonly DependencyProperty RichTextContentProperty = DependencyProperty.Register("RichTextContent",
		...
		/// </summary>
		public static readonly DependencyProperty ColumnTemplateProperty = DependencyProperty.Register("ColumnTemplate",
			typeof (DataTemplate),
			typeof (RichTextColumns),
			new PropertyMetadata(null, ResetOverflowLayout));
		...
		/// </summary>
		public RichTextBlock RichTextContent { get { return (RichTextBlock) GetValue(RichTextContentProperty); } set { SetValue(RichTextContentProperty, value); } }

		/// <summary>
		///    Gets or sets the template used to create additional
		///    <see cref="RichTextBlockOverflow" /> instances.
		/// </summary>
		public DataTemplate ColumnTemplate { get { return (DataTemplate) GetValue(ColumnTemplateProperty); } set { SetValue(ColumnTemplateProperty, value); } }
		...
		protected override Size MeasureOverride(Size availableSize)
		{
			...
			while (hasOverflow && maxWidth < availableSize.Width && ColumnTemplate != null)
			{
				...
				if (_overflowColumns.Count > overflowIndex) overflow = _overflowColumns[overflowIndex];
				else
				{
					overflow = (RichTextBlockOverflow) ColumnTemplate.LoadContent();
					_overflowColumns.Add(overflow);
					Children.Add(overflow);
					if (overflowIndex == 0) RichTextContent.OverflowContentTarget = overflow;
					else _overflowColumns[overflowIndex - 1].OverflowContentTarget = overflow;
				}
			...
			{
				if (overflowIndex == 0) RichTextContent.OverflowContentTarget = null;
				else _overflowColumns[overflowIndex - 1].OverflowContentTarget = null;
